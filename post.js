...
router.post('/setTable/:userID',  function (req, res) {
    ...
    db.DeleteByColumn('tableList', { 'userID': userID }, function() {

        for(i = 0; i < req.body['now_table'].length; i++){

            inputData['userID']=userID;
            inputData['courseID']=req.body['now_table'][i];
            inputData['userName']="汪曉東C";

            db.Insert('tableList', inputData, function(err, results){
                if (err) throw err;
            });
        }
